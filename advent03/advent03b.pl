...
my $wire1 = <DATA>; chomp $wire1;
my $wire2 = <DATA>; chomp $wire2;
...
($x, $y) = (100,100);
my $wire1steps = 0;
foreach my $move (split ',', $wire1) {
  my ($dir, $len) = $move =~ /([UDLR])(\d+)/;
  for (1..$len) {
    $wire1steps++;
    $x++ if $dir eq 'R';
    $x-- if $dir eq 'L';
    $y++ if $dir eq 'U';
    $y-- if $dir eq 'D';
    $wire1points->{"$x,$y"} = $wire1steps;
  }
}

($x, $y) = (100,100);
my $wire2steps = 0;
foreach my $move (split ',', $wire2) {
  my ($dir, $len) = $move =~ /([UDLR])(\d+)/;
  for (1..$len) {
    $wire2steps++;
    $x++ if $dir eq 'R';
    ...
    $y++ if $dir eq 'U';
    $y-- if $dir eq 'D';
    $wire2points->{"$x,$y"} = $wire2steps;
  }
}

my $shortest;
foreach my $pos (keys %$wire1points) {
  next unless $wire2points->{$pos};
  my $steps = $wire1points->{$pos} + $wire2points->{$pos};
  $shortest = $steps if !$shortest || $steps < $shortest;
}

print "Fewest steps to an intersecting point is $shortest\n";
