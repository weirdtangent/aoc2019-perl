...
for my $noun (0..99) {
  for my $verb (0..99) {
    ...
    $tryit->[1] = $noun;
    $tryit->[2] = $verb;
    my $result = run_intcode($tryit);

    die "Found $looking_for with noun=$noun and verb=$verb. Coded as ".($noun * 100 + $verb)."\n"
      if $result == $looking_for;
  }
}

die "Never found $looking_for !?\n";
...
sub run_intcode {
  ...
  while ($iptr < scalar(@$code) && $code->[$iptr] != 99) {
    $code->[$code->[$iptr+3]] = $code->[$code->[$iptr+1]] + $code->[$code->[$iptr+2]], $iptr += 4 if $code->[$iptr] eq 1;
    $code->[$code->[$iptr+3]] = $code->[$code->[$iptr+1]] * $code->[$code->[$iptr+2]], $iptr += 4 if $code->[$iptr] eq 2;
  }
